...
Input_Video = "video/10.mp4"
Input_Video1 = "video/19.mp4"

Input_image = cv2.imread("image/10.png")
Input_image1 = cv2.imread("image/19.png")

Init_image = background(Input_image)
Init_image1 = background(Input_image1)
...
def main():
    ...
    cap1 = cv2.VideoCapture(Input_Video1)

    #10 cctv
    vertices = [[[380, 480], [380, 780], [1540, 780], [1540, 480]]] #가로
    #vertices = [[[1160, 950], [1500, 950], [1500, 320], [1160, 320]]] #세로

    #19 cctv
    vertices1 = [[[600, 480], [500, 780], [1450, 780], [1350, 480]]]


    thread = threading.Thread(target=counting, args=(cap,vertices,1, ))
    thread1 = threading.Thread(target=counting, args=(cap1,vertices1,0,))
    thread.start()
    thread1.start()
    ...
    counting(cap, vertices, 1)
    counting(cap1, vertices1, 0)
    ...
    if initBB is not None:
        ...
        if success:
            ...
            if (Matched == True):
                ...
                global count #전역변수 사용을 위한 global 선언

                if intersect(initBB_xy, Matched_xy, RED_line_start_xy, RED_line_end_xy):
                    RED_cnt = RED_cnt + 1
                    if inout == 1: # 같은 red line을 사용할 때 in count와 out count 구분
                        count += 1
                    else:
                        count -= 1
...
def counting(cap, vertices, inout):
    fps = FPS().start()
    ...
    while(cap.isOpened()):
        ...
        if f_num % 2== 0 and f_num>0:
            ...
            difference = cv2.absdiff(Init_image, gray_frame)
            ...
            tracker_1, initBB_1, RED_cnt_1, BLUE_cnt_1 = Passing_Counter_Zone(Vehicle_x, Vehicle_y, Vehicle_w, Vehicle_h, initBB_1, frame, tracker_1, Substracted,\
                                      RED_cnt_1, BLUE_cnt_1, vertices, inoutt)
            ...
            if inout == 1:
                ...
                cv2.line(frame, (vertices[0][0][0], vertices[0][0][1]), (vertices[0][3][0], vertices[0][3][1]), (255, 0, 0), 2)
                cv2.putText(frame, "OUT Cnt : " + str(RED_cnt_1), (vertices[0][0][0], vertices[0][0][1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 3)
                #일방 통행이기 때문에 똑같은 red count 이지만 이름만 out count로 설정
            ...
            cv2.putText(frame, "Car count : " + "{}".format(count), (500, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,(255, 255, 255), 2)
