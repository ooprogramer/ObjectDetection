...
Image1 = cv2.imread("image/10.png")
Image2 = cv2.imread("image/19.png")
...
def main():
    ...
    initBB_1 = None
    tracker_1 = None
    vertices1 = [[[380, 480], [380, 780], [1540, 780], [1540, 480]]]
    #19
    Detecting_cnt_2 = 0
    RED_cnt_2 = 0
    BLUE_cnt_2 = 0
    initBB_2 = None
    tracker_2 = None
    vertices2 = [[[600, 480], [500, 780], [1450, 780], [1350, 480]]]

    while True:
        f_num =f_num +1
        print("F : ", f_num)

        (grab1, frame1) = cap1.read()
        (grab2, frame2) = cap2.read()
...
#frame1start--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            difference = cv2.absdiff(gray1, gray_frame)
            ...
            Substracted1 = cv2.bitwise_and(frame1, mask3)

            layerOutputs, start, end = YOLO_Detect(frame1)

            # 3.YOLO_BOX_INFO(layerOutputs,BaseConfidence,Base_threshold))
            idxs, boxes, classIDs, confidences = YOLO_BOX_INFO(frame1, layerOutputs, BaseConfidence, Base_threshold)

            # 4.검출된 화면의 X,Y 좌표 가져온다. 검출됨 차량 수 만큼 좌표 가져옴
            ...
            Draw_Points(frame1, Vehicle_x, Vehicle_y, Vehicle_w, Vehicle_h)

            tracker_1, initBB_1, RED_cnt_1, BLUE_cnt_1 = Passing_Counter_Zone(Vehicle_x, Vehicle_y, Vehicle_w, Vehicle_h, initBB_1, frame1, tracker_1, Substracted1,\
                                      RED_cnt_1, BLUE_cnt_1, vertices1, 1)

            # Red_Line
            cv2.line(frame1, (vertices1[0][0][0], vertices1[0][0][1]), (vertices1[0][3][0], vertices1[0][3][1]), COLOR_RED, 2)
            cv2.putText(frame1, "IN Cnt : " + str(RED_cnt_1), (vertices1[0][0][0], vertices1[0][0][1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, COLOR_RED, 3)

            # Blue_Line
            #cv2.line(frame1, (vertices[0][1][0], vertices[0][1][1]), (vertices[0][2][0], vertices[0][2][1]), (255, 0, 0), 2)
            #cv2.putText(frame1, "IN Cnt : " + str(BLUE_cnt_1), (1vertices[0][1][0], 1vertices[0][1][1] + 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 3)

            # Detecting Zone
            pts_1 = detecting_zone(vertices1)
            cv2.polylines(frame1, [pts_1], True, COLOR_GREEN, 2)

            #프레임 레터박스
            blank_image = np.zeros((64, 1920, 3), np.uint8)
            frame1[0:64, 0:1920] = blank_image

            frame1 = cv2.resize(frame1, (800, 450), interpolation=cv2.INTER_CUBIC) #1920, 1080 -> 1280,720
            Substracted1 = cv2.resize(Substracted1 , (800, 450), interpolation=cv2.INTER_CUBIC)

            fps.update()
            fps.stop()
            cv2.putText(frame1, "FPS : " + "{:.2f}".format(fps.fps()) + "          " + "Car count : " + "{}".format(count), (25, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7,COLOR_WHITE, 1)
#frame1end--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
#frame2start--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            ...
            difference = cv2.absdiff(gray2, gray_frame)
            _, difference = cv2.threshold(difference, 35, 255, cv2.THRESH_BINARY)

            mask3 = cv2.cvtColor(difference, cv2.COLOR_GRAY2BGR)  # 3 channel mask
            Substracted2 = cv2.bitwise_and(frame2, mask3)

            layerOutputs, start, end = YOLO_Detect(frame2)

            # 3.YOLO_BOX_INFO(layerOutputs,BaseConfidence,Base_threshold))
            idxs, boxes, classIDs, confidences = YOLO_BOX_INFO(frame2, layerOutputs, BaseConfidence, Base_threshold)
            ...
            #차량 포인트 그리기
            Draw_Points(frame2, Vehicle_x, Vehicle_y, Vehicle_w, Vehicle_h)

            tracker_2, initBB_2, RED_cnt_2, BLUE_cnt_2 = Passing_Counter_Zone(Vehicle_x, Vehicle_y, Vehicle_w, Vehicle_h, initBB_2, frame2, tracker_2, Substracted2,\
                                      RED_cnt_2, BLUE_cnt_2, vertices2, 0)

            # BLUE_Line
            cv2.line(frame2, (vertices2[0][0][0], vertices2[0][0][1]), (vertices2[0][3][0], vertices2[0][3][1]), COLOR_BLUE, 2)
            cv2.putText(frame2, "OUT Cnt : " + str(RED_cnt_2), (vertices2[0][0][0], vertices2[0][0][1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 1, COLOR_BLUE, 3)

            # RED_Line
            #cv2.line(frame2, (vertices[0][1][0], vertices[0][1][1]), (vertices[0][2][0], vertices[0][2][1]), (0, 0, 255), 2)
            #cv2.putText(frame, "IN Cnt : " + str(BLUE_cnt_1), (vertices[0][1][0], vertices[0][1][1] + 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)

            # Detecting Zone
            pts_2 = detecting_zone(vertices2)
            cv2.polylines(frame2, [pts_2], True, COLOR_GREEN, 2)

            frame2[0:64, 0:1920] = blank_image

            frame2 = cv2.resize(frame2, (800, 450), interpolation=cv2.INTER_CUBIC) #1920, 1080 -> 1280,720
            Substracted2 = cv2.resize(Substracted2, (800, 450), interpolation=cv2.INTER_CUBIC)

            fps.update()
            fps.stop()
            cv2.putText(frame2, "FPS : " + "{:.2f}".format(fps.fps()) + "          " + "Car count : " + "{}".format(count), (25, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7,COLOR_WHITE, 1)
#frame2end--------------------------------------------------------------------------------------------------------------------------------------------------------------------------
            frame = cv2.hconcat([frame1, frame2])
            Substracted = cv2.hconcat([Substracted1, Substracted2])
            output = cv2.vconcat([frame, Substracted])
    ...
    if len(idxs) > 0:
        ...
        for i in idxs.flatten():
            ...
            if classIDs[i] == 2 or classIDs[i] == 7:
                ...
                Vehicle_x.append(x); Vehicle_y.append(y); Vehicle_w.append(w); Vehicle_h.append(h) #frame에 검출된 모든 차량 좌표 입력
...
def Draw_Points(frame,Vehicle_x,Vehicle_y,Vehicle_w,Vehicle_h):
    if len(Vehicle_x) > 0:
        for i in range(0, len(Vehicle_x), 1):
            cv2.circle(frame, (Vehicle_x[i] + int(Vehicle_w[i] / 2), Vehicle_y[i] + Vehicle_h[i]), 5, COLOR_GREEN, -1)
            #cv2.putText(frame, 'car', (Vehicle_x[i] + int(Vehicle_w[i] / 2) - 10, Vehicle_y[i] + Vehicle_h[i] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
            cv2.rectangle(frame, (Vehicle_x[i], Vehicle_y[i]), (Vehicle_x[i]+Vehicle_w[i], Vehicle_y[i]+Vehicle_h[i]), COLOR_RED, 2)



def Passing_Counter_Zone(Vehicle_x,Vehicle_y,Vehicle_w,Vehicle_h,initBB,frame,tracker,Substracted,RED_cnt,BLUE_cnt,vertices,inout):
    # Detecting Zone
    ...
    if initBB is not None:
        ...
        if success:
            ...
            cv2.putText(frame, str(car_cnt), (x+int(w/2)-10, y+h-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
